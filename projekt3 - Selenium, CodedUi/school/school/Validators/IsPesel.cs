    ...
    public class IsPesel : ValidationAttribute
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            if (validationContext.DisplayName == null)
                errorMessage = "Number PESEL is uncorecte";
            else
            ...
            if (pesel.Length != 11
            ) // jeżeli ciąg posiada więcej lub mniej niż 11 znaków wyświetlamy komunikat o błędzie
            ...
            for (int i = 0; i < pesel.Length; i++)
            {
                ...
                ) // jeżeli któryś ze znaków nie jest cyfrą wyświetlamy komunikat o błędzie
                    return new ValidationResult(errorMessage);

                if (i + 1 == pesel.Length)
                {
                    if ((10 - k % 10) % 10 != temp
                    ) // jeżeli suma kontrolna jest niepoprawna wyświetlamy komunikat o błędzie
                        return new ValidationResult(errorMessage);
                }
                else
                    k += temp * weight[i];
